...
# =========================
# Notes:
# 1. This is the vision-layer local cooldown, reduces duplicate triggers.
# 2. DGHUB plugin has single-layer cooldown (vision-local only).
...
def send_trigger(event_name, level_text):
    """
    Send trigger event to the backend.

    In normal DGHUB plugin operation, this function is monkey-patched
    by main.py to send trigger messages via DGHUB WebSocket.
    If this stub is reached, the monkey-patch has not been applied
    (e.g. running vision standalone without the plugin entry point).

    Returns False so callers can retry on failure.
    """
    print("[WARN] send_trigger() stub called — monkey-patch not applied!")
    print("[WARN] event=" + event_name + " level=" + level_text)
    return False
